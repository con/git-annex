...
- A workflow on con/git-annex-ci-client-jobs is established that is run
...
- A table (generated by `mktable.py`) is added to the main README in this
  repository displaying the SVG badges from con/git-annex-ci-client-jobs.
...
- On the client:
    ...
    - Clone this repository.  Passing `--single-branch` is recommended so as
      not to include the mirror of git-annex's repository. E.g.:
        ...
        ```shell
        git clone --single-branch http://github.com/con/git-annex ~/git-annex-ci/git-annex
        ```
...
- Edit the `README.md` in the root of this repository to add badges for the
  ...
  successfully processed by con/git-annex-ci-client-jobs.)
